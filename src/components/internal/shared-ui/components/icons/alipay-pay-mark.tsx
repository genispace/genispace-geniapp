import * as React from 'react';
import { cn } from '@genispace/shared-utils';

/** Alipay mark (glyph). Paths from TDesign Icons (Tencent), MIT License. */
const ALIPAY_GLYPH_MAIN =
  'M1.592 4.993v14.014c0 1.824 1.492 3.304 3.335 3.304h14.147c1.842 0 3.333-1.48 3.333-3.304v-.143c-.061-.026-5.41-2.236-8.127-3.53c-1.834 2.235-4.2 3.591-6.655 3.591c-4.153 0-5.563-3.6-3.597-5.97c.43-.517 1.158-1.01 2.29-1.287c1.771-.43 4.59.27 7.23 1.133a14.3 14.3 0 0 0 1.173-2.84H6.58v-.816h4.197V7.681H5.695v-.817h5.083v-2.09s0-.351.36-.351h2.052v2.44h5.025v.818H13.19v1.464h4.102a16.5 16.5 0 0 1-1.737 4.347q.664.238 1.27.46c.732.268 1.388.507 1.926.686c2.778.92 3.556 1.033 3.657 1.045V4.993c0-1.825-1.492-3.304-3.334-3.304H4.927c-1.843 0-3.335 1.479-3.335 3.304';

const ALIPAY_GLYPH_ACCENT =
  'M6.672 12.891q.27-.026.537-.03c1.723-.019 3.364.664 5.269 1.585c-1.478 1.909-3.358 3.101-5.24 3.101c-3.239 0-4.196-2.527-2.596-3.909c.533-.468 1.51-.695 2.03-.747';

export function AlipayPayMark({
  className,
  ...props
}: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 24 24"
      aria-hidden
      className={cn('shrink-0', className)}
      {...props}
    >
      <path fill="#1677FF" d={ALIPAY_GLYPH_MAIN} />
      <path fill="#1677FF" d={ALIPAY_GLYPH_ACCENT} />
    </svg>
  );
}
